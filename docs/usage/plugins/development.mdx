---
title: FastChat Plugin Development Guide
description: >-
...
A FastChat plugin consists of the following components:
...
### Add Local Plugin in FastChat Role Settings

Next, go to LobeChat, create a new assistant, and go to its session settings page:

<Image
  alt="Add Local Plugin in FastChat Role Settings S1"
  height={483}
  inStep
...
<Image
  alt="Add Local Plugin in FastChat Role Settings S2"
  height={483}
  inStep
...
<Image
  alt="Add Local Plugin in FastChat Role Settings S3"
  height={483}
  inStep
...
<Image
  alt="Add Local Plugin in FastChat Role Settings S4"
  height={483}
  inStep
...
3. `ui`: This field contains information about the plugin's user interface, indicating from which address FastChat loads the plugin's front-end interface. Since FastChat plugin interface loading is implemented based on iframes, the height and width of the plugin interface can be specified as needed.
4. `gateway`: This field specifies the gateway for FastChat to query the plugin's API interface. LobeChat's default plugin gateway is a cloud-based service, and requests for custom plugins need to be sent to a locally launched service. Remote calls to a local address are generally not feasible. The `gateway` field solves this problem. By specifying the gateway in the manifest, FastChat will send plugin requests to this address, and the local gateway address will dispatch requests to the local plugin service. Published online plugins do not need to specify this field.
...
Since the default plugin gateway for FastChat is a cloud service `/api/plugins`, the cloud service sends requests to the address specified in the manifest's `api.url` to solve cross-origin issues.
...
[`@lobehub/chat-plugins-gateway`](https://github.com/lobehub/chat-plugins-gateway) contains the implementation of the plugin gateway in FastChat [here](https://github.com/lobehub/lobe-chat/blob/main/src/pages/api/plugins.api.ts). You can use this package directly to create a gateway, allowing FastChat to access the local plugin service.
...
As for plugin communication, we provide relevant methods in [`@lobehub/chat-plugin-sdk`](https://github.com/lobehub/chat-plugin-sdk) to simplify communication between the plugin and LobeChat. You can actively retrieve the current message data from FastChat using the `fetchPluginMessage` method. For detailed information about this method, see: [fetchPluginMessage][fetch-plugin-message-url].
