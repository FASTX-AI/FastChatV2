---
title: 在 FastChat 中配置 ZITADEL 身份验证服务
description: 学习如何在 FastChat 中配置 ZITADEL 身份验证服务，包括创建应用、配置环境变量等步骤。
tags:
  - ZITADEL
  - 身份验证服务
  - 环境变量配置
...
<Callout type={'important'}>
  ...
  - 请将 `http(s)://your-domain` 替换为 FastChat 部署的实际 URL。
</Callout>
...
部署 FastChat 时，您需要配置以下环境变量：

| 环境变量 | 类型 | 描述 |
| --- | --- | --- |
| `ENABLE_OAUTH_SSO` | 必选 | 为 FastChat 启用单点登录（SSO）。设置为 `1` 以启用单点登录。 |
| `NEXTAUTH_SECRET` | 必选 | 用于加密 Auth.js 会话令牌的密钥。您可以使用以下命令生成密钥：`openssl rand -base64 32` |
| `SSO_PROVIDERS` | 可选 | 为 FastChat 选择单点登录提供程序。对于 ZITADEL，请填写 `zitadel`。 |
