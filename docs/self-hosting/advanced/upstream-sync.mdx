---
title: FastChat Upstream Sync and Docker Deployment Guide
description: >-
  Learn how to enable automatic updates for FastChat Vercel deployments and
...
Upgrading the Docker deployment version is very simple, you just need to redeploy the latest FastChat image. Here are the commands required to perform these steps:

<Steps>
  ### Stop and Remove the Current Running FastChat Container

Assuming the FastChat container is named `lobe-chat`, use the following commands to stop and remove the currently running FastChat container:
...
### Pull the Latest FastChat Image
...
Redeploy the FastChat container using the newly pulled image:
...
No need to worry, you won't. All of LobeChat's chat records are stored in your local browser. Therefore, when redeploying FastChat using Docker, your chat records will not be lost.
